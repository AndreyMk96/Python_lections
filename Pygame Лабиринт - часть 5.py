"""Игра лабиринт чать 5. Генерация и сбор монеток"""
import random
...
coin_points = []
for i in range(5):
    coin_points.append([random.randint(100,700), random.randint(100,600)])
# Создаем игру и окно
pygame.init()
pygame.mixer.init()
...
while running:
    ...
    # Добавляем несколько монеток
    for i in range(len(coin_points)):
        coin = pygame.draw.circle(screen, "yellow", (coin_points[i][0], coin_points[i][1]), 15)
        collide2 = coin.colliderect(smile)
        if collide2:
            if list(coin.center) in coin_points:
                coin_points.remove(list(coin.center))
                break
